...
import {
  ...
  useTheme, Avatar, Badge, Chip, Popover, List, ListItem,
  ListItemAvatar, ListItemText, Divider, Button,
} from '@mui/material';
import {
  Brightness4, Brightness7, Notifications, NavigateNext,
  Logout, AdminPanelSettings, FiberManualRecord,
...
const AdminNavbar = () => {
  ...
  const handleLogout = () => { handleProfileClose(); logout(); navigate('/admin/login', { replace: true }); };
  ...
  return (
    <>
        ...
        <Toolbar sx={{ justifyContent: 'space-between', minHeight: '64px !important', px: { xs: 2, md: 3 } }}>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Box sx={{ display: { xs: 'none', sm: 'flex' }, alignItems: 'center', gap: 0.5 }}>
              <RouterLink to="/admin/dashboard" style={{ textDecoration: 'none', color: theme.palette.text.secondary, fontSize: '0.8rem', fontWeight: 500 }}>
                Admin
              </RouterLink>
          ...
          <Box display="flex" alignItems="center" gap={0.5}>
            ...
            <Chip
              ...
              size="small"
              sx={{ fontSize: '0.72rem', display: { xs: 'none', md: 'flex' }, fontWeight: 600, bgcolor: isDark ? '#1e1e1e' : '#f5f5f5' }}
            />
            ...
            <Tooltip title="Admin Profile">
              <Box onClick={handleProfileOpen} sx={{
                display: 'flex', alignItems: 'center', gap: 1, ml: 0.5, pl: 1.5, pr: 1,
                py: 0.6, borderRadius: 3, cursor: 'pointer',
                border: `1px solid ${isDark ? '#2a2a2a' : '#efefef'}`,
                '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.05)' : '#fafafa' },
                transition: 'all 0.2s',
              }}>
                <Box sx={{ position: 'relative' }}>
                  <Avatar alt="Admin" src={avatarImg} sx={{ width: 32, height: 32, border: '2px solid #b71c1c' }} />
      ...
      <Popover
        open={Boolean(notifAnchor)} anchorEl={notifAnchor} onClose={handleNotifClose}
        anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
        transformOrigin={{ vertical: 'top', horizontal: 'right' }}
        PaperProps={{ sx: { width: 340, borderRadius: 3, mt: 1, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', border: `1px solid ${isDark ? '#2a2a2a' : '#f0f0f0'}` } }}
      >
          ...
          {sampleNotifications.map((n, i) => (
            <React.Fragment key={n.id}>
              <ListItem sx={{ py: 1.5, px: 2 }}>
                <ListItemAvatar>
                  <Avatar sx={{ bgcolor: `${n.color}20`, color: n.color, width: 38, height: 38, fontSize: '0.85rem', fontWeight: 800, border: `1.5px solid ${n.color}44` }}>
                    {n.avatar}
                  </Avatar>
                </ListItemAvatar>
                <ListItemText
      ...
      {/* Profile Popover */}
      <Popover
        open={Boolean(profileAnchor)} anchorEl={profileAnchor} onClose={handleProfileClose}
        anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
        transformOrigin={{ vertical: 'top', horizontal: 'right' }}
        PaperProps={{ sx: { width: 272, borderRadius: 3, mt: 1, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', overflow: 'hidden', border: `1px solid ${isDark ? '#2a2a2a' : '#f0f0f0'}` } }}
      >
        <Box sx={{ p: 2.5, background: 'linear-gradient(135deg, #7f0000 0%, #b71c1c 100%)', display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ position: 'relative' }}>
            <Avatar alt="Admin" src={avatarImg} sx={{ width: 52, height: 52, border: '2px solid rgba(255,255,255,0.4)' }} />
          ...
          <Box>
            ...
            <Chip
              label="Administrator"
              size="small"
              icon={<AdminPanelSettings sx={{ fontSize: '0.7rem !important', color: '#fff !important' }} />}
        ...
        <Box sx={{ p: 1.5 }}>
          <Button fullWidth startIcon={<Logout fontSize="small" />} onClick={handleLogout}
            sx={{
              borderRadius: 2, textTransform: 'none', fontWeight: 600, fontSize: '0.83rem',
              py: 1, justifyContent: 'flex-start', color: 'error.main',
