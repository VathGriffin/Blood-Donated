  ...
  return (
    <>
      ...
      >
        <Toolbar sx={{ justifyContent: "space-between", height: "100%", px: { xs: 2, md: 3 } }}>
          ...
          <NavLink to="/" style={{ display: "flex", alignItems: "center", textDecoration: "none" }}>
            <img src={logo} alt="Blood Logo" style={{ width: 48, height: 48, borderRadius: "50%", objectFit: "cover" }} />
            <Typography
              variant="h5"
              sx={{ ml: 1.5, color: "white", fontWeight: 800, letterSpacing: "-0.5px", fontSize: { xs: "1.3rem", md: "1.6rem" } }}
            >
              Blood Donated
            </Typography>
          </NavLink>
          ...
          {!isMobile ? (
              ...
              {sections.map(({ label, path, icon }) => (
                <NavLink
                  ...
                  onMouseLeave={(e) => {
                    ...
                    e.currentTarget.style.backgroundColor = active ? "rgba(255,255,255,0.2)" : "transparent";
                  }}
                >
                  {icon}
                  <span>{label}</span>
              ...
              {isAuth ? (
                <>
                  <Tooltip title="Messages">
                    <IconButton component={NavLink} to="/messages" sx={{ color: "white" }}>
                      <Chat fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  ...
                  >
                    ...
                    <MenuItem onClick={() => { navigate("/messages"); handleUserMenuClose(); }}>
                ...
                <Box sx={{ display: "flex", gap: 1 }}>
                  <Button
                    component={NavLink} to="/login" size="small" variant="outlined"
                  ...
                  <Button
                    component={NavLink} to="/signup" size="small" variant="contained"
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              ...
              >
                ...
                <MenuItem onClick={() => { navigate("/messages"); handleUserMenuClose(); setDrawerOpen(false); }}>
                  <Chat fontSize="small" sx={{ mr: 1.5 }} /> My Messages
                </MenuItem>
                <MenuItem onClick={handleLogout} sx={{ color: "error.main" }}>
                  <Logout fontSize="small" sx={{ mr: 1.5 }} /> Log Out
      ...
      <Drawer
        ...
        PaperProps={{
          sx: { width: 280, backgroundColor: theme.palette.mode === "dark" ? "#1a0000" : "#fff", pt: 2 },
        }}
      >
        ...
        <List sx={{ pt: 1 }}>
          {sections.map(({ label, path, icon }) => (
            ...
            >
              {({ isActive }) => (
                <ListItem sx={{ ...mobileNavStyle({ isActive }), py: 1.2 }}>
                  <ListItemIcon sx={{ minWidth: 40 }}>{icon}</ListItemIcon>
                  <ListItemText
                    primary={label}
                    primaryTypographyProps={{ fontWeight: isActive ? 700 : 500, color: isActive ? "error.main" : "text.primary" }}
                  />
                  {isActive && <Box sx={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: "#b71c1c", mr: 1 }} />}
                </ListItem>
              )}
            </NavLink>
          ))}

          {/* Messages link in mobile drawer */}
          <NavLink to="/messages" style={{ textDecoration: "none", color: "inherit" }} onClick={() => setDrawerOpen(false)}>
            {({ isActive }) => (
              <ListItem sx={{ ...mobileNavStyle({ isActive }), py: 1.2 }}>
                <ListItemIcon sx={{ minWidth: 40 }}><Chat sx={{ color: "#26a69a" }} /></ListItemIcon>
                <ListItemText primary="Messages" primaryTypographyProps={{ fontWeight: isActive ? 700 : 500, color: isActive ? "error.main" : "text.primary" }} />
              </ListItem>
            )}
          </NavLink>
        </List>
        ...
        <Box sx={{ px: 2, pt: 2, pb: 1 }}>
          {isAuth ? (
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              <Button fullWidth variant="contained" color="error" component={NavLink} to="/login"
                onClick={() => setDrawerOpen(false)} sx={{ borderRadius: 2, textTransform: "none", fontWeight: 700 }}>
                Log In
              </Button>
              <Button fullWidth variant="outlined" color="error" component={NavLink} to="/signup"
