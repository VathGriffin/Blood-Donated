...
const UserMessages = () => {
  ...
  const border = `1px solid ${isDark ? "#2a2a2a" : "#f0f0f0"}`;
  ...
  if (!isAuth) {
    return (
      <Box sx={{ minHeight: "70vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", px: 2, textAlign: "center", backgroundColor: bg }}>
        ...
        <Typography variant="h5" fontWeight={700} gutterBottom>Please log in to message admin</Typography>
        <Typography variant="body2" color="text.secondary" mb={3}>You need an account to send messages.</Typography>
  ...
  return (
    <Box sx={{ backgroundColor: bg, minHeight: "100vh" }}>
      <Box sx={{
        ...
        <Typography variant="body1" sx={{ opacity: 0.88 }}>Have a question? Chat directly with our team.</Typography>
      </Box>

      <Box sx={{ maxWidth: 700, mx: "auto", px: 2, py: 5 }}>
        <Paper elevation={4} sx={{ borderRadius: 4, overflow: "hidden", display: "flex", flexDirection: "column", height: 560 }}>
          <Box sx={{ px: 3, py: 2, borderBottom: border, display: "flex", alignItems: "center", gap: 1.5, backgroundColor: panelBg }}>
            <Avatar sx={{ bgcolor: "#b71c1c", width: 38, height: 38, fontSize: "0.85rem", fontWeight: 700 }}>AD</Avatar>
            <Box>
          ...
          <Box ref={messagesBoxRef} sx={{ flex: 1, overflowY: "auto", px: 3, py: 2, display: "flex", flexDirection: "column", gap: 1.5, backgroundColor: bg }}>
            {loading ? (
              ...
              <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" height="100%" gap={1}>
                ...
                <Typography color="text.secondary" fontSize="0.9rem">No messages yet. Say hello!</Typography>
              </Box>
            ) : messages.map((msg) => {
                ...
                <Box key={msg._id} sx={{ display: "flex", justifyContent: isUser ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 1 }}>
                  ...
                  <Box sx={{ maxWidth: "72%" }}>
                    <Box sx={{
                      ...
                      backgroundColor: isUser ? "#b71c1c" : panelBg,
                  ...
                  {isUser && <Avatar sx={{ bgcolor: "#424242", width: 30, height: 30, fontSize: "0.7rem", fontWeight: 700, mb: 0.5 }}>{user?.fullName?.charAt(0)?.toUpperCase()}</Avatar>}
                </Box>
              );
            })}
          </Box>

          <Box sx={{ px: 3, py: 2, borderTop: border, display: "flex", gap: 1.5, backgroundColor: panelBg, alignItems: "flex-end" }}>
            <TextField
              fullWidth
              multiline
              maxRows={4}
            ...
            <IconButton
              ...
              sx={{
                bgcolor: "#b71c1c", color: "white", width: 44, height: 44, borderRadius: 2.5, flexShrink: 0,
